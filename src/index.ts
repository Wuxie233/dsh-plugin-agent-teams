...
4. Later turns use agent_teams_send_message naming a returned task id and instructions. Default delivery is queue: a running recipient finishes the current turn first. Pass mode=barge only to interrupt immediately. One task per message keeps turns focused.
5. Yield after dispatch. Member reports and stall notices wake you. agent_teams_status is for a snapshot, a stall, or collecting completed outputs — not a wait loop. If a member reports a blocker, reassign the task or adjust the plan.
