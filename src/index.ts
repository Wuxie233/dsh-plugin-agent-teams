...
2. Call agent_teams_add_member once per role the goal needs (researcher, engineer, reviewer, ...). Members are durable subagents: they wait for your messages, then work a full turn. By default each member snapshots your current provider, model, and reasoning effort. Never ask the user to choose these per member; only pass provider/model when the user explicitly requests a different route for that role. For a write-isolated member, create a git worktree first (git worktree add) and pass its absolute path as worktree; read-only roles refuse worktrees, and a member's worktree is fixed at spawn, so plan tasks per member-tree and merge member trees back in dependency order (the captain resolves merge conflicts).
