...
2. Break the goal into tasks with agent_teams_create_task; wire dependencies between tasks (a task is claimable only when its dependencies are completed). Assign each task to a member when it fits a role. Default writers share the captain workspace with exclusive path ownership. Only when two writers must edit the same files in parallel, or the change must stay abortable, create a git worktree first and plan those tasks as per-member lanes — a member's worktree is frozen at spawn.
...
4. Dispatch work: claim each assigned task (agent_teams_claim_task with assignee) and barge in on the member with agent_teams_send_message naming its task id and instructions. A running recipient is interrupted so the message starts immediately. One task per message keeps turns focused.
